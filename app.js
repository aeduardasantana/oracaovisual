...
const steps=[
 {purpose:'PRESENÇA DE DEUS',title:'Na presença da Santíssima Trindade',subtitle:'Um só Deus em três Pessoas: Pai, Filho e Espírito Santo. Cada Pessoa tem seu papel e todas estão sempre presentes nesta oração.',items:trinity,type:'person',multi:false,fixed:true,trinityPanel:true},
...
function render(){
 ...
 const trinityPanel=step.trinityPanel?`<div class="trinity-panel"><img src="assets/trindade/santissima-trindade.webp?v=20260916-2" alt="Santíssima Trindade: Deus Pai, Jesus Cristo e Espírito Santo"><div>${trinity.map(item=>`<article><strong>${item.label}</strong><span>${item.meaning}</span></article>`).join('')}</div></div>`:'';
...
function renderConfessionGuidance(){
 const box=$('#confessionGuidance'),marked=allStepItems(2).filter(item=>choices[2].includes(item.id));
 if(!marked.length){box.innerHTML='';return}
 const prayers=[
  {title:'MEU JESUS',text:'Meu Jesus, meu bom Jesus, que por mim morreste na cruz, perdoa os meus pecados. Já não quero mais pecar. Amém.'},
  {title:'CONFESSO A DEUS',note:'Ato penitencial da Missa',text:'Confesso a Deus todo-poderoso e a vós, irmãos, que pequei muitas vezes por pensamentos e palavras, atos e omissões. (Bate-se no peito) Por minha culpa, minha culpa, minha tão grande culpa. E peço à Virgem Maria, aos Anjos e Santos, e a vós, irmãos, que rogueis por mim a Deus, nosso Senhor. Amém.'},
  {title:'MEU DEUS',text:'Meu Deus, porque sois infinitamente bom e Vos amo de todo o meu coração, pesa-me de Vos ter ofendido e, com o auxílio da Vossa divina graça, proponho firmemente emendar-me e nunca mais Vos tornar a ofender. Peço e espero o perdão das minhas culpas pela Vossa infinita misericórdia. Amém.'}
 ];
