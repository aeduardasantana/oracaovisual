...
function render(){
 ...
 const trinityPanel=step.trinityPanel?`<div class="trinity-panel"><img src="assets/trindade/santissima-trindade.webp" alt="Santíssima Trindade: Deus Pai, Jesus Cristo e Espírito Santo"><div>${trinity.map(item=>`<article><strong>${item.label}</strong><span>${item.meaning}</span></article>`).join('')}</div></div>`:'';
