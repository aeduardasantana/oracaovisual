...
const steps=[
 ...
 {purpose:'PEDIR PERDÃO',title:'Por que você quer pedir perdão?',subtitle:'Escolha se desejar. Você também pode seguir sem marcar nada.',items:sins,multi:true,groups:[{label:'MEU DIA',items:sins},{label:'10 MANDAMENTOS',items:commandments},{label:'PRECEITOS DA IGREJA',items:churchPrecepts}]},
...
let current=0,saintName='',choices=steps.map(()=>[]),activeGroups=steps.map(()=>0);choices[0]=['pai','jesus','espirito'];
...
function render(){
 const step=steps[current],visibleItems=step.groups?step.groups[activeGroups[current]].items:step.items;$('#progressBar').style.width=`${((current+1)/steps.length)*100}%`;
 const groupTabs=step.groups?`<div class="group-tabs">${step.groups.map((group,index)=>`<button type="button" class="${activeGroups[current]===index?'active':''}" data-group="${index}">${group.label}</button>`).join('')}</div>`:'';
 content.innerHTML=`<div class="step-head"><span class="step-number">${step.purpose} · PASSO ${current+1} DE ${steps.length}</span><h2>${step.title}</h2><p>${step.subtitle}</p></div>${groupTabs}<div class="cards">${visibleItems.map(item=>`<button type="button" class="visual-card ${step.type||''} ${step.fixed?'fixed':''} ${choices[current].includes(item.id)?'selected':''}" data-id="${item.id}" style="--pos:${item.pos||'50% 50%'}" aria-pressed="${choices[current].includes(item.id)}"><span class="check">✓</span><span class="art" role="img" aria-label="${item.label}">${item.icon||''}</span><span class="label">${item.label}</span><span class="meaning">${item.meaning}</span><span class="image-credit">${current===0?'Ilustração original':'Símbolo visual'}</span></button>`).join('')}</div>${step.saintField?`<div class="saint-field"><label for="saintName">OUTRO SANTO DE DEVOÇÃO</label><input id="saintName" maxlength="60" value="${saintName}" placeholder="Exemplo: Santa Teresinha do Menino Jesus"><small>Opcional. O nome será usado apenas nesta oração.</small></div>`:''}`;
 content.querySelectorAll('[data-group]').forEach(button=>button.addEventListener('click',()=>{activeGroups[current]=Number(button.dataset.group);render()}));
