const people=[
 {id:'pai',label:'DEUS PAI',meaning:'Pai que ama',pos:'52% 4%'},{id:'jesus',label:'JESUS',meaning:'Filho amado, nosso irmão',pos:'50% 49%'},{id:'espirito',label:'ESPÍRITO SANTO',meaning:'Amor, Paráclito e Advogado',pos:'70% 27%'},{id:'maria',label:'SANTA MARIA',meaning:'Intercede junto a Jesus',pos:'8% 42%'},{id:'jose',label:'SÃO JOSÉ',meaning:'Intercede junto a Jesus',pos:'94% 44%'}
];
const adoration=[
...
const steps=[
 {purpose:'PRESENÇA DE DEUS',title:'Com quem você quer começar?',subtitle:'Dirija-se a Deus ou peça a intercessão de Santa Maria ou São José.',items:people,type:'person',multi:false},
...
let current=0,choices=steps.map(()=>[]);
...
function render(){
 ...
 content.innerHTML=`<div class="step-head"><span class="step-number">${step.purpose} · PASSO ${current+1} DE ${steps.length}</span><h2>${step.title}</h2><p>${step.subtitle}</p></div><div class="cards">${step.items.map(item=>`<button type="button" class="visual-card ${step.type||''} ${choices[current].includes(item.id)?'selected':''}" data-id="${item.id}" style="--pos:${item.pos||'50% 50%'}" aria-pressed="${choices[current].includes(item.id)}"><span class="check">✓</span><span class="art" aria-hidden="true">${item.icon||'•'}</span><span class="label">${item.label}</span><span class="meaning">${item.meaning}</span></button>`).join('')}</div>`;
 content.querySelectorAll('.visual-card').forEach(card=>card.addEventListener('click',()=>select(card.dataset.id)));back.style.visibility=current===0?'hidden':'visible';next.textContent=current===steps.length-1?'MONTAR MINHA ORAÇÃO →':'CONTINUAR →';next.disabled=choices[current].length===0;
}
...
function buildPrayer(){
 const person=people.find(item=>item.id===choices[0][0]);
 const opening=['maria','jose'].includes(person.id)?`${person.label}, intercede junto a Jesus por mim e acompanha esta oração.`:person.id==='espirito'?'Espírito Santo, Amor de Deus, Paráclito e Advogado, vem em meu auxílio.':`${person.label}, eu me coloco em tua presença.`;
 const resolution=resolutions.find(item=>item.id===choices[6][0]);
 const paragraphs=[opening,`Eu te adoro e louvo: ${sentence(texts(adoration,1))}.`,`Senhor, reconheço que ${sentence(texts(sins,2))}. Tem misericórdia de mim e perdoa-me.`,`Eu te agradeço ${sentence(texts(thanks,3))}.`,`Senhor, eu te peço: ${sentence(texts(virtues,4))}.`,`Eu intercedo: ${sentence(texts(intercessions,5))}.`,`${resolution.meaning}. Espírito Santo, dá-me a graça da perseverança para cumprir este propósito.`];
